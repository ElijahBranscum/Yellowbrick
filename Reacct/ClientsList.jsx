...
function ClientsList(props) {
  ...
  _logger("prpps:", props.currentUser.roles);
  ...
  useEffect(() => {
    ...
    } else if (clients.dependencyCount > 0) {
      clientService
        .getById(
          clients.currentPage - 1,
          clients.pageSize,
  ...
  const onGetByCurrentIdSuccess = (response) => {
    setClients((prevState) => {
      const toggleUI = { ...prevState };
      toggleUI.clientList = response.item.pagedItems;
      toggleUI.clientRows = response.item.pagedItems.map(clientMap);
      toggleUI.totalCount = response.item.totalCount;
        ...
        <div className="col-lg-12 col-md-12 col-sm-12">
          <div className="border-bottom mb-4 d-md-flex align-items-center justify-content-between">
            <div className="mb-3 mb-md-0">
              ...
              {clients.isBtnsActive && (
                <a className="active maintain-cursor" onClick={onFilterClick}>
                  Filter By Agent
                </a>
              )}
            </div>
            {clients.isFilterActive && clients.isBtnsActive ? (
              <div>
